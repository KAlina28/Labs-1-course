...
TEST_CASE("Archive test") {
    ...
    SUBCASE("Empty file") {
        archiver.archieve("test/archive_test/empty.txt", output);
        CHECK(compare(output, "test/archive_test/empty_after.bin"));
    }
    SUBCASE("One letter file") {
        archiver.archieve("test/archive_test/one_letter.txt", output);
        CHECK(compare(output, "test/archive_test/one_letter_after.bin"));
    }
    SUBCASE("Some txt file") {
        archiver.archieve("test/archive_test/sometxt.txt", output);
        CHECK(compare(output, "test/archive_test/sometxt_after.bin"));
    }
}